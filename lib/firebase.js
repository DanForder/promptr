...
export const getRandomPrompt = async () => {
  ...
  // if we get an item that's equal or higher than than guid, use it
  // otherwise wrap-around to the lowest item in the collection
  const q = query(
  ...
  const docOne = (await getDocs(q)).docs[0];
  ...
  const docTwo = (await getDocs(backupQ)).docs[0];
